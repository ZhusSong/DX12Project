...
void GameApp::BuildModels()
{
    ...
    ModelTest= ModelManager("asset\\Models\\player.obj");

    auto meshGeometry = std::make_unique<MeshGeometry>();
    std::vector<SkinnedVertex> vertices;
    std::vector<UINT> indices;

    for (UINT i = 0; i < ModelTest.m_RenderInfo.size(); i++)
    {
        SubmeshGeometry submesh;
        submesh.BaseVertexLocation = vertices.size();
        submesh.StartIndexLocation = indices.size();
        submesh.IndexCount = ModelTest.m_RenderInfo[i].indices.size();

        vertices.insert(vertices.end(), ModelTest.m_RenderInfo[i].vertices.begin(), ModelTest.m_RenderInfo[i].vertices.end());
        indices.insert(indices.end(), ModelTest.m_RenderInfo[i].indices.begin(), ModelTest.m_RenderInfo[i].indices.end());

        meshGeometry->DrawArgs["Player"]=submesh;
    }
    ...
    meshGeometry->VertexByteStride = sizeof(SkinnedVertex);
...
void GameApp::BuildMaterials()
{
    ...
    for (UINT i = 0; i < ModelTest.m_Materials.size(); i++)
    {
        // 创建模型材质
        auto modelMat= std::make_unique<Material>();
        modelMat->MatCBIndex = i;
        modelMat->DiffuseSrvHeapIndex = ModelTest.m_Materials[i].diffuseMaps;
        modelMat->FresnelR0 = DirectX::XMFLOAT3(0.02f, 0.02f, 0.02f);
        modelMat->Roughness = 0.3f;
        modelMat->DiffuseAlbedo = DirectX::XMFLOAT4(1.0f, 1.0f, 1.0f, 1.0f);
        mMaterials["PlayerMat"] = std::move(modelMat);
    }
  /*  auto modelMat = std::make_unique<Material>();
    modelMat->Name = "playerMat";
    modelMat->MatCBIndex = 5;
    modelMat->DiffuseSrvHeapIndex = ModelTest.mat;
...
void GameApp::BuildRenderItems()
{
    ...
    for (UINT i = 0; i < ModelTest.m_RenderInfo.size(); i++)
    {
        DirectX::XMStoreFloat4x4(&modelRitem->TexTransform, DirectX::XMMatrixIdentity());

        modelRitem->ObjCBIndex = 3;
        modelRitem->Geo = mGeometries["Player"].get();
        modelRitem->Mat = mMaterials["PlayerMat"].get();
