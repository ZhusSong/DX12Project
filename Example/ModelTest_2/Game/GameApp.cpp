...
void GameApp::BuildModels()
{
    ...
    uint32_t k = 0; 
    for (size_t m = 0; m < ModelVertices.size(); m++)
    {
        for (size_t i = 0; i < ModelVertices[m].size(); i++, k++)
        {
            localVertices[k].position =  ModelVertices[m][i].position;
            localVertices[k].normal = ModelVertices[m][i].normal;
            localVertices[k].texCoord = ModelVertices[m][i].texCoord;
        }
    }
    std::vector<uint32_t> localIndices;
    localIndices.insert(localIndices.end(), std::begin(ModelIndices), std::end(ModelIndices));

    const uint32_t vbSize = localVertices.size() * sizeof(ModelVertex);
    const uint32_t ibSize = localIndices.size() * sizeof(uint32_t);
